...
基于 **TileLayout** 构建的**纯文本表格（CSV）本地编辑器**，底层数据是人类可读的纯文本，演示瓦片引擎在数据可视化与编辑交互场景中的应用。

- 数据：内置**化学元素周期表**（118 种元素 × 13 个维度）默认数据集，纯文本 CSV 存储于私有目录，首次启动从 assets 落盘，右上角菜单可一键重置
- 渲染：`MeasurableDimenProvider` 自动测量列宽/行高；表头与正文两种 TextView 样式各占一个瓦片类型；正文行按奇偶**斑马纹**着色；缩放矢量不失真
- 编辑：点击单元格弹 Material3 输入框，保存即同步落盘；长按弹菜单（删除本列/本行、左移/右移列、上移/下移行）
- 文本：底部「复制全文」（TSV/CSV 二选一复制到剪贴板）与「解析」（TSV/CSV 二选一全量替换）；新建列/行自动 seek 过去查看
...
- 算法：`SplitMix64` 哈希确定性布雷（密度约 15.6%）、BFS 展开空白区、15 种状态外观矢量预录制
...
基于 **TileView（Canvas 绘制）** 的康威生命游戏（Conway's Game of Life），运行于**完整 int32 无限平面**。

- 规则：标准 B3/S23——活细胞邻居 2~3 存活，死细胞邻居恰好 3 复活
- 数据：稀疏活细胞表（`列<<32 | 行` 打包为 long 键），死细胞不占存储，内存只与活细胞数量相关；每代计算只遍历活细胞及其邻居，复杂度与平面大小无关
- 线程：单后台线程逐代演算（纯函数 + 快照 copy-on-write），主线程零阻塞；编辑/暂停/清空通过代次令牌作废在途结果
- 渲染：每代只 `update()` 状态或年龄档位发生变化的坐标（增量重绘），变化超过 256 格时降级为一次全量刷新；4 档年龄着色（新生亮青 → 长寿紫），5 种外观 `Picture` 矢量预录制，缩放不重录
- 图案：内置 9 种经典图案——滑翔机、轻型飞船、脉冲星、**高斯帕滑翔机枪**（每 30 代发射一架滑翔机，无限增长）、R-五格骨牌（混沌 1103 代）、橡果（5206 代）、顽固份子（130 代消亡）、蟾蜍、方块
- 场景：点击细胞放置/移除、单步/连续演化（5 档速度）、视窗内随机播撒、清空；进入稳定态或全部消亡时自动停止
