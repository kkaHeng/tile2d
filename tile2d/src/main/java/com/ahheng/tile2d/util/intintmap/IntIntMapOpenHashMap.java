...
public class IntIntMapOpenHashMap implements IntIntMap {
    ...
    private int nullValue; // 0 键对应的值(哨兵)
    private int maxFill; // 触发扩容的元素个数上限

    public IntIntMapOpenHashMap() {
        this(DEFAULT_INITIAL_SIZE);
    }

    public IntIntMapOpenHashMap(int expected) {
    ...
    public int get(int k) {
        ...
        int curr;
        int pos = mix(k) & mask;
        if ((curr = key[pos]) == 0) return 0;
        if (k == curr) return value[pos];
        while (true) {
            if ((curr = key[pos = (pos + 1) & mask]) == 0) return 0;
            if (k == curr) return value[pos];
        }
    }
    ...
    public int get(int k, int defaultValue) {
        ...
        int curr;
        int pos = mix(k) & mask;
        if ((curr = key[pos]) == 0) return defaultValue;
        if (k == curr) return value[pos];
        while (true) {
